...
public final class TropicubeFallenKingdoms extends JavaPlugin {
    ...
    @Override public void onEnable() {
        saveDefaultConfig();
        try { settings = FallenKingdomsSettings.load(getConfig()); }
    ...
    @Override public void onDisable() { if (stateMachine.state().active()) stateMachine.transitionTo(GameState.ENDING); }
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        else if (action.equals("start")) message(sender, stateMachine.transitionTo(GameState.COUNTDOWN) ? "fk.countdown-started" : "fk.transition-refused");
        else if (action.equals("cancel")) message(sender, stateMachine.transitionTo(GameState.WAITING) ? "fk.countdown-cancelled" : "fk.transition-refused");
        else if (action.equals("stop")) { stateMachine.transitionTo(GameState.ENDING); message(sender, "fk.stopped"); }
