...
`min-instances` est le plancher créé dès l'initialisation de Velocity puis maintenu toutes les 30 secondes par l'autoscaler. Les templates Lobby, SheepWars Quick Play et Fallen Kingdoms Quick Play utilisent un plancher de `1` ; une instance de chacun est donc démarrée avec le proxy. Pour SheepWars, le matchmaking crée également une nouvelle instance à la demande lorsqu'aucun serveur classique en attente ou en démarrage n'est encore joignable. `max-instances` borne toutes les créations simultanées, qu'elles viennent du matchmaking, d'une partie personnalisée ou de `/tropi start`.
