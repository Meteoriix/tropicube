...
`min-instances` est le plancher créé dès l'initialisation de Velocity puis maintenu toutes les 30 secondes par l'autoscaler. La ressource embarquée utilise un plancher de `1` pour Lobby, SheepWars Quick Play et Fallen Kingdoms Quick Play. Le profil Docker de développement conserve seulement Lobby et SheepWars à ce plancher ; Fallen Kingdoms démarre à la demande afin que son instance classique ou une de ses files bêta puisse tenir dans le budget local. Pour SheepWars, le matchmaking crée également une nouvelle instance à la demande lorsqu'aucun serveur classique en attente ou en démarrage n'est encore joignable. `max-instances` borne toutes les créations simultanées, qu'elles viennent du matchmaking, d'une partie personnalisée ou de `/tropi start`.
...
`scoreboards.yml` et `tablists.yml` définissent les variantes `waiting`, `countdown`, `active` et `ending`. Le scoreboard actif affiche la phase actuelle, la prochaine échéance et uniquement les équipes affectées, sans PV de cœur. La tablist porte la carte et colore les pseudonymes selon la préférence puis l'affectation. Pendant une phase active, l'actionbar affiche les PV actuels et maximaux du cœur allié ; une alerte de réapparition ou de territoire prend temporairement la priorité. Ces ressources rejoignent le bundle UI de Core et se rechargent avec les langues. La migration `V011__fallenkingdoms_preferences.sql` conserve le dernier kit et la dernière couleur préférée de chaque joueur.
...
La configuration Docker livrée fixe `docker.memory-budget-mib` à `8192` Mio pour le poste de développement ; la ressource embarquée conserve `16384` Mio pour les installations autonomes. Dans ce profil local, Lobby et SheepWars réservent chacun 2560 Mio, marge native comprise, et une seule instance Fallen Kingdoms classique ou bêta réserve les 2560 Mio restants. Fallen Kingdoms a donc `auto-start: false` et `min-instances: 0` dans la copie Docker. Cette différence est intentionnelle : adapter la copie Docker à la mémoire réellement disponible avant une installation serveur.
