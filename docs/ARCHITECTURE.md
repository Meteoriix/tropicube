...
Le tableau de bord Profil appartient à Core et fournit une tête de joueur localisée marquée par données persistantes sur Java, remplacée par une étiquette stable sur Bedrock pour éviter la tête de Steve. Lobby place cette entrée au slot 4 et SheepWars au slot 7 uniquement durant l'attente ; Core reste l'unique gestionnaire du clic. Lobby enregistre une passerelle bornée pour ouvrir ses Paramètres depuis ce tableau de bord. Tous les inventaires Core, Lobby et SheepWars réutilisent `NetworkMenuStyle` sans dépendance métier entre jeux. Le style conserve le cadre gris/aqua et les scintillements décoratifs sur Java, mais détecte Geyser par la marque client ou le préfixe Floodgate et supprime les vitres ainsi que les faux enchantements sur Bedrock : les actions restent ainsi isolées dans l'interface tactile sans afficher d'enchantement trompeur. Le pack intégré Geyser complète cette adaptation et son mapping de textures traduit les icônes HeadDatabase statiques.

Le sélecteur de jeux route les clics sans accès bloquant : gauche vers Quick Play, droite vers le choix Ranked 4v4/8v8 et `Maj + clic gauche` vers une liste filtrée des instances publiques. La catégorie `BETA` ouvre un écran dédié qui adresse directement les templates expérimentaux et permet de quitter la file active ; elle propose actuellement Fallen Kingdoms 1v1 et 2v2. Ces noms indiquent le seuil par royaume : les parties peuvent réunir de deux à cinq royaumes et démarrent dès que chacun compte respectivement au moins un ou deux joueurs. Velocity publie pour chaque template et instance un `InstanceMode` (`QUICK_PLAY`, `RANKED_4V4`, `RANKED_8V8` ou `CUSTOM`) ; Lobby ne déduit donc pas la nature d'une partie à partir de son nom. Les parties personnalisées privées restent invisibles hors whitelist et leur création visible dans le sélecteur est verrouillée sous la priorité de grade VIP+.
...
La hotbar conserve `0 Jeux`, `2 Social`, `4 Profil`, `8 Boutique` ; Jeux et Social emploient le vert dans les quatre langues. Le sélecteur principal de jeux utilise le cadre `neutral`, sans ligne de vitres bleues, tandis que les autres écrans réseau conservent leur accent aqua. Le Profil garde l'identité en 13, Missions en 20, Notifications en 22, Paramètres en 31 et expose les passerelles Lobby facultatives : Vestiaire en 24, Progression en 30, Guide en 32. Sans Lobby, ces passerelles sont absentes ; le centre Core reste utilisable dans l'attente SheepWars.
