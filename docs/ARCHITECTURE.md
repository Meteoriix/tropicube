...
L'éditeur local conserve les ressources Maven comme source de vérité et leurs miroirs Docker comme source de build. Il partage le rendu des placeholders nommés via `tropicube-language-api`. Les dispositions des menus, scoreboards et tablists résident dans des manifestes versionnés propres à leur module. Après validation, l'éditeur installe langues et manifestes dans les conteneurs, puis appelle `languageeditorreload <request-id>` par RCON. Core ou Velocity remplace son catalogue en mémoire et écrit un marqueur temporaire de réussite ou d'échec ; l'éditeur attend ce marqueur avant de compter le conteneur comme rechargé. Core publie chaque ensemble sous une nouvelle génération Redis (`runtime-ui:generation:<id>:*`) et ne remplace `runtime-ui:active` qu'après les fichiers et leur manifeste de hashes. Une nouvelle instance vérifie puis restaure cette génération avant de charger ses gestionnaires.

Fallen Kingdoms conserve dans son module le calcul des jours, les alertes de cœur et les coffres progressifs. `GameDay`, `LootTables` et `LootRoller` sont des modèles Java purs ; `LootChestService` charge les chunks de carte sans bloquer puis repasse sur le scheduler Paper pour valider et modifier blocs ou inventaires. Les positions de carte appartiennent à `MapDefinition`, les coffres actifs sont marqués par données persistantes et leurs protections restent des adaptateurs Paper minces. Aucun de ces composants ne dépend du métier SheepWars ni ne justifie une extraction vers Core.

Le HUD Fallen Kingdoms construit un scoreboard personnel par observateur. Ses équipes utilisent le nom réel du profil pour les entrées client, tandis que le texte visible peut conserver le nick. La dépendance `glowingentities`, isolée et relocalisée dans le JAR FK, applique le contour uniquement entre alliés et est nettoyée avec la session. Les alertes d'actionbar respectent l'ordre réapparition, territoire, attaque du cœur, puis état normal.

La restauration vérifie le hash du fichier Redis d'origine puis complète les langues Core dans un fichier temporaire avec les clés manquantes du JAR. Les anciennes valeurs officielles ayant fait l'objet d'une migration explicite sont remplacées par leur nouvelle version ; toute valeur différente, donc personnalisée dans l'éditeur, reste intacte. Le remplacement atomique précède le chargement des langues. Les clés Redis, leur persistance sans expiration et le pointeur de génération ne changent pas ; la restauration ne republie pas la génération. Cette étape empêche une ancienne génération de réintroduire des clés manquantes ou des textes officiels retirés après la migration initiale des configurations.
...
7. Une instance vide et éligible à `auto-stop` est arrêtée après `auto-stop-delay`, sans descendre sous `min-instances`. Seul le template Lobby maintient une instance dès le démarrage du proxy ; SheepWars et Fallen Kingdoms sont créés à la demande.
...
La machine à états pilote préparation, JcJ, assaut, mort subite et résultat. Les adaptateurs Paper appliquent les protections territoriales, les dégâts de cœur, les vies, la reconnexion, la ruine bornée, les menus de royaume/kit/carte et le HUD localisé. La salle d'attente suit la navigation SheepWars, mais son manifeste `menus.yml` et ses choix restent propres au module. Core persiste les résultats dans les tables génériques `tropicube_game_results`, `tropicube_game_result_players` et `tropicube_game_statistics`; l'identifiant de partie rend l'écriture idempotente. Après la persistance, le canal Redis `PROXY:FINISH_GAME:<instance>` rend l'instance à Velocity. Le domaine ne dépend pas de SheepWars.
...
| `player:uuid:<pseudo>` / `player:name:<uuid>` | Velocity | Velocity/SheepWars | Résolution de l'identité visible et des membres de whitelist déjà vus ; TTL 30 jours renouvelé à la connexion |
| `player:authenticated-name:<uuid>` | Velocity | Core | Nom authentifié indépendant de `/nick`, utilisé par les listes d'amis et les données persistantes ; TTL 30 jours renouvelé à la connexion |
...
Core recopie le pseudonyme et le grade factice d'une identité `/nick` dans un cache visuel local lors de `NICK_APPLY`. Ce cache alimente la tablist et les annonces d'entrée du lobby, tandis que le chat lit la même identité Redis ; aucun de ces affichages ne remplace le grade réel utilisé pour les permissions. Pour `NICK_CLEAR`, Velocity conserve `nick:<uuid>` et `nick:original:<uuid>` jusqu'à ce que le backend possédant le joueur ait restauré son profil ; ce backend supprime ensuite les deux clés hors du thread Paper. La demande reste ainsi rejouable si un message Pub/Sub est perdu ou si le joueur change de serveur. Après avoir actualisé le profil, `displayName`, `playerListName` et la visibilité de l'entité, Core émet localement `PlayerDisplayIdentityChangedEvent`. Les interfaces des mini-jeux utilisent cet événement achevé plutôt que la demande Redis concurrente pour reconstruire leurs tablists et équipes de scoreboard. Le cache est vidé à la désactivation du nick ou au déchargement du joueur, puis restauré depuis `nick:<uuid>` à la reconnexion. Le lobby résout le nom visible depuis `Player#displayName`, actualisé immédiatement par Core à l'activation comme à la désactivation, car le nom du profil Paper peut rester transitoirement obsolète après `setPlayerProfile`.

SheepWars et Fallen Kingdoms consomment `PlayerDisplayIdentityChangedEvent` après une activation ou une suppression de nick, puis réappliquent deux ticks plus tard leur rendu local et leurs entrées de scoreboard. Les événements Redis `GRADE_LOADED` et `GRADE_CHANGED` restent responsables des changements de grade. La tablist SheepWars ne reprend aucun préfixe ni icône : le `displayName` synchronisé est coloré selon l'équipe, ou en gris pour un spectateur. Le chat utilise cette même identité afin de suivre immédiatement `/nick off`. Les équipes de scoreboard continuent séparément d'utiliser le nom de profil envoyé au client pour le contour des entités.

Velocity mémorise le nom authentifié reçu dans `GameProfileRequestEvent` pendant toute la connexion. Les commandes administratives qui doivent cibler l'identité réelle, notamment les suggestions et la résolution de `/pull`, consultent ce cache local sans accès Redis et ne dépendent donc pas du `GameProfile` remplacé par `/nick`.
...
Un changement de langue publie `LANG_CHANGED:<uuid>:<langue>` sur le canal joueurs. Le lobby reconstruit alors, sur le thread Paper, la hotbar, le scoreboard personnel et la tablist. Le scoreboard du lobby est donc entièrement localisé et reste cohérent que la langue soit changée depuis le menu ou avec `/lang`. La tablist conserve la marque Tropicube, affiche le libellé simple `Lobby` et utilise l'accent violet du réseau pour l'adresse du serveur.

Le tableau de bord Profil appartient à Core et fournit une tête de joueur localisée marquée par données persistantes sur Java, remplacée par une étiquette stable sur Bedrock pour éviter la tête de Steve. Lobby place cette entrée au slot 4 et SheepWars au slot 7 uniquement durant l'attente ; Core reste l'unique gestionnaire du clic. Lobby enregistre une passerelle bornée pour ouvrir ses Paramètres depuis ce tableau de bord. Tous les inventaires Core, Lobby et SheepWars réutilisent `NetworkMenuStyle` sans dépendance métier entre jeux. Le style conserve un cadre gris sans ligne de vitres bleues et les scintillements décoratifs sur Java, mais détecte Geyser par la marque client ou le préfixe Floodgate et supprime les vitres ainsi que les faux enchantements sur Bedrock : les actions restent ainsi isolées dans l'interface tactile sans afficher d'enchantement trompeur. Le pack intégré Geyser complète cette adaptation et son mapping de textures traduit les icônes HeadDatabase statiques.

Le sélecteur de jeux route les clics sans accès bloquant : gauche vers Quick Play, droite vers le choix Ranked 4v4/8v8 et `Maj + clic gauche` vers une liste filtrée des instances publiques. Si aucun template Ranked n'existe pour le jeu, le menu reste ouvert et affiche un message localisé d'indisponibilité. La catégorie `BETA` ouvre un écran dédié qui adresse directement les templates expérimentaux et permet de quitter la file active ; elle propose actuellement Fallen Kingdoms 1v1 et 2v2. Ces noms indiquent le seuil par royaume : les parties peuvent réunir de deux à cinq royaumes et démarrent dès que chacun compte respectivement au moins un ou deux joueurs. Velocity publie pour chaque template et instance un `InstanceMode` (`QUICK_PLAY`, `RANKED_4V4`, `RANKED_8V8` ou `CUSTOM`) ainsi que le type de jeu et le format destinés à l'affichage. Le scoreboard de file montre alors `jeu • icône • format`, avec 🌴 pour Quick Play, ⚔ pour Ranked, 🧪 pour Beta et ⭐ pour Custom ; le format est omis lorsqu'il est absent. Les parties personnalisées privées restent invisibles hors whitelist et leur création visible dans le sélecteur est verrouillée sous la priorité de grade VIP+.
...
## Gestion des guildes depuis Social

Le Lobby présente les onglets Amis, Groupe et Guildes dans les emplacements 2, 4 et 6 de Social. Le bouclier Guildes remplace l'ancien accès du Profil ; l'appartenance reste visible dans la fiche du joueur. Les écrans de guilde restent dans Lobby et utilisent les services Core pour consulter les invitations, membres, contributions, défis et classement, ou effectuer les mutations. Les membres sont paginés par 21 et seules les têtes nécessaires à la page sont résolues. Java conserve les profils ; Bedrock reçoit les icônes vanilla adaptées.

Les écritures d'administration et la succession automatique acquièrent un verrou MySQL nommé `tc:guild:<UUID dérivé du nom de la base>`, partagé entre instances et limité à cinq secondes d'attente, puis utilisent la même connexion transactionnelle pour vérifier et modifier l'état. Le verrou est explicitement libéré avant restitution au pool ; aucune modification du schéma ni nouvelle clé Redis n'est nécessaire. Les contributions de gameplay ne prennent pas ce verrou. Les actions de menu transmettent l'identifiant de guilde affiché : un écran périmé ne peut pas agir sur une nouvelle guilde du joueur.

Core intercepte les saisies privées avant les filtres et la publication du chat réseau. Une saisie n'est pas un message public, y compris pour un joueur muet. Le nom, puis le tag, ou le pseudo à inviter sont consommés une seule fois et traités sur le thread Paper. `!` annule ; une nouvelle navigation, la déconnexion ou l'arrêt annulent aussi le contexte. Les accès SQL et les résolutions de profils restent asynchrones. Les réponses de chargement ne s'appliquent qu'à l'inventaire qui les a demandées ; fermer ou changer d'écran les rend inactives. `/lang` actualise l'écran Guildes et l'étape de saisie en cours sans réinitialiser son délai.

## Admission, limites et observabilité

Core initialise SQL, Redis, la restauration des interfaces et le catalogue de grades sur un worker de cycle de vie. Les dépendants Lobby/SheepWars attendent `initializeBackend` : leurs connexions Redis sont préparées hors du thread Paper, puis leurs composants Paper sont construits sur le scheduler serveur. Le garde d'admission reste fermé jusqu'à la réussite de cette phase. Le marqueur `TROPICUBE_BACKEND_READY` remplace le message générique `Done` pour les nouvelles créations Velocity. Déployer donc Core, les deux backends et Velocity dans le même lot. Un arrêt du backend referme immédiatement l'admission ; les callbacks de démarrage tardifs sont ignorés.

Le SQL soumis à DatabaseManager passe par un exécuteur avec concurrence et file bornées. Une saturation complète exceptionnellement la future sans bloquer l'appelant. La déconnexion capture les données sur Paper et sauvegarde hors thread ; les permissions Bukkit sont retirées sur Paper. Le chargement d'un profil libère sa connexion de lecture avant d'appeler les autres repositories, évitant l'acquisition imbriquée de connexions. Une erreur de chargement déconnecte le joueur avec un message localisé au lieu de le laisser sans profil.

DockerManager réserve atomiquement la mémoire avant création et la conserve si la suppression du conteneur échoue. À l'initialisation il reconstruit les réservations depuis les limites Docker réelles, même si elles dépassent un budget nouvellement réduit. La file classique conserve les joueurs lorsque seule la mémoire manque et réessaie après cinq secondes. Un dépassement ne déclenche aucune suppression de partie active.

Les nouveaux conteneurs et volumes portent `fr.tropicube.owner`, égal au préfixe configuré. Le nettoyage ignore les ressources d'un autre propriétaire. Pour les ressources historiques sans label, les noms doivent correspondre au préfixe ; les conteneurs doivent aussi appartenir au réseau configuré. Cela permet une pile de validation avec un préfixe et un réseau distincts sur le même daemon.

La table additionnelle `tropicube_schema_checksums` utilise `version` comme clé primaire et enregistre nom de ressource et SHA-256 avant son exécution. Les fins de ligne CRLF sont normalisées en LF pour la portabilité. Une empreinte divergente bloque le démarrage, y compris après une interruption DDL. L'historique préexistant n'est adopté que si le nom correspond et si la ressource correspond au manifeste historique versionné. Les migrations MySQL ne deviennent pas transactionnelles : leur reprise doit rester idempotente et testée. Le verrou de session `tropicube:core:schema` est partagé avec la sauvegarde SQL.

Les clés de diagnostic sont `tropicube:health:proxy` et `tropicube:health:backend:<instanceId>`, mises à jour toutes les 60 secondes par remplacement atomique `SET EX`, TTL 180 secondes. Le proxy publie nombre d'instances, minimum attendu, taille de file et mémoire réservée ; Core publie opérations SQL actives/en attente et disponibilité. L'outil local les lit et agrège les backends sans exposer de données joueur. Ces clés ne servent pas à conserver les parties ; après restauration, les références d'instances sont réconciliées avec Docker comme auparavant.

## Parcours Profil et découverte

La hotbar conserve `0 Jeux`, `2 Social`, `4 Profil`, `8 Boutique` ; Jeux et Social emploient le vert dans les quatre langues. Tous les écrans réseau utilisent un cadre gris sans ligne de vitres bleues. Le Profil garde l'identité en 13, Missions en 20, Notifications en 22, Paramètres en 31 et expose les passerelles Lobby facultatives : Vestiaire en 24, Progression en 30, Guide en 32. Sans Lobby, ces passerelles sont absentes ; le centre Core reste utilisable dans l'attente SheepWars.

Les récompenses de missions réclamables et notifications non lues sont rechargées à chaque ouverture et affichées dans les descriptions. Les lectures affichent un inventaire de chargement distinct ; fermer ou changer de menu invalide leur résultat. Les retours d'action ne rafraîchissent que l'écran encore actif. Les icônes du Profil respectent le rendu Java/Bedrock de NetworkMenuStyle, sans supposer qu'une icône est toujours une tête.

Le clic gauche sur un jeu lance directement sa partie Quick Play : SheepWars rejoint sa file et les autres jeux demandent le démarrage de leur template. Les raccourcis Java clic droit et Maj + clic gauche ouvrent respectivement Classé et Parties publiques ; l'entrée Parties personnalisées reste disponible. Les modes utilisent les templates publiés par Velocity et le routage existant ; aucune règle SheepWars ne change. Les restrictions classées restent détaillées dans le sélecteur existant.
...
Le Vestiaire est une interface Lobby enregistrée auprès du Profil Core comme les Paramètres, puis retirée à l'arrêt. Il propose Traînées/Sons, les filtres Tous/Disponibles/Verrouillés, des pages de 21 objets, une fiche avec condition d'accès, aperçu et équipement/déséquipement. L'aperçu ne modifie ni sélection ni acquisition. Les sélections ne sont jamais attribuées automatiquement. Le Guide Personnalisation ouvre le même vestiaire. Les choix par niveau ou VIP sont revérifiés avant équipement ; un VIP perdu suspend l'effet sans effacer la sélection, toujours déséquipable.

Core possède CosmeticCatalog et CosmeticService, leurs instantanés immuables et les transactions SQL. Les consultations utilisent un instantané REPEATABLE READ sur une connexion worker ; l'équipement verrouille la ligne joueur, puis remplace une seule catégorie dans une transaction. Les résultats métier sont explicites, les pannes d'infrastructure restent des futures exceptionnelles. La migration V009 ajoute les clés primaires `(player_uuid, cosmetic_id)` et `(player_uuid, category)` ; les clés étrangères suppriment ces données lors de l'anonymisation existante. L'export inclut les acquisitions et sélections. Aucun nouveau contrat Redis n'est introduit.
...
Les chargements existants de Boutique, Paramètres et Classé possèdent un écran annulable et un contrôle d'identité de l'inventaire. Paramètres attend indépendamment Redis pour l'auto-replay et MySQL pour les préférences, avec une limite de cinq secondes par source ; toute panne ou expiration affiche Réessayer au lieu de laisser le chargement actif. Les retours depuis Classé et Parcourir ramènent au sélecteur de jeux. `/lang` reconstruit les écrans des nouveaux parcours, et Core rafraîchit son propre Profil même sans Lobby. Les sous-menus de notification et de navigation publique placent Retour à gauche et Fermer à droite ; Langues place aussi Fermer à droite.
