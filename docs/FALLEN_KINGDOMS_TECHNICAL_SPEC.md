...
- Le kit n'est distribué qu'une fois, au début de la partie ; 32 steaks sont ensuite ajoutés sans écraser ses emplacements.
...
kits:
  ...
  alchemist:
    ...
    icon: BREWING_STAND
    items:
      - { material: BREWING_STAND, amount: 1, slot: 0 }
      - { material: BLAZE_POWDER, amount: 4, slot: 1 }
      - { material: NETHER_WART, amount: 8, slot: 2 }
      - { material: GLASS_BOTTLE, amount: 6, slot: 3 }
      - { material: SUGAR, amount: 4, slot: 4 }
      - { material: SPIDER_EYE, amount: 4, slot: 5 }
...
La sélection de carte, équipe et kit reste graphique. La hotbar d'attente reprend le parcours SheepWars : équipe, kit et carte aux emplacements 0 à 2, lancement ou annulation pour l'hôte en 4, Profil en 7 et retour au lobby en 8. Les objets affichent le choix courant et le clic attendu. Les inventaires partagent le cadrage réseau, placent le retour à gauche et la fermeture à droite, distinguent le choix actif et présentent les demandes par équipe, le contenu réel des kits et le nombre de votes. Chaque objet, variante de potion et enchantement du kit possède un libellé dans les quatre langues ; les quantités agrégées et les piles sont formulées selon `/lang`, sans exposer de clé Minecraft. Une commande refusée ne modifie jamais partiellement la session. Les textes sont fournis en `fr`, `en`, `de` et `es` via Adventure/MiniMessage.

Le scoreboard d'attente affiche l'effectif courant sur la capacité totale de la session, calculée comme le maximum par royaume multiplié par le nombre maximal de royaumes. Le scoreboard actif affiche le jour calculé par `elapsed / 600 + 1`, borné de 1 à 6, la phase actuelle, le nom et le délai de la prochaine phase, la couleur du royaume du joueur et uniquement les royaumes affectés, sans PV de cœur. Les noms visibles des royaumes sont les couleurs seules, par exemple « Bleu » au lieu de « Équipe bleue ». Chaque scoreboard personnel crée une équipe par royaume avec le nom réel du profil comme entrée ; les spectateurs sont gris et les nicks visuels restent compatibles. La tablist porte la carte et colore les pseudonymes selon la préférence en attente puis l'affectation réelle. `glowingentities` rend uniquement les alliés vivants lumineux pour leur propre royaume et le rendu est reconstruit aux changements de cycle de vie ou d'identité. L'actionbar affiche en continu les PV actuels et maximaux du cœur allié pendant les phases actives ; les alertes contextuelles de réapparition ou de territoire la remplacent temporairement. Après un dégât réellement appliqué au cœur, une alerte blanche/rouge et `minecraft:entity.blaze.hurt` préviennent ses membres en ligne, avec durée, cadence et anti-spam configurables. Un coup valide sur un cœur ennemi affiche à l'attaquant une bossbar pendant cinq secondes depuis le dernier coup.
...
| Construction dans sa base | autorisée sauf blocs techniques | identique | identique | identique |
| Construction en zone commune | autorisée sauf blocs techniques | identique | identique | identique |
| Pose manuelle en base ennemie | interdite | interdite | TNT uniquement | TNT uniquement |
| Casse manuelle en base ennemie | interdite | interdite | interdite | interdite |
...
Les conteneurs et établis sont utilisables par les participants actifs. Les portillons d'une base restent réservés à son équipe, même pendant l'assaut ; une base ruinée devient neutre. Une tentative d'entrée ennemie avant l'assaut produit une alerte localisée limitée à une occurrence toutes les deux secondes.
