...
The world template is stored in Paper 26.2's current storage format. The Overworld data directory must retain `minecraft/world_gen_settings.dat`, `paper/metadata.dat`, and `paper/level_overrides.dat`. Removing any of them makes every ephemeral copy look like a vanilla world import, triggers the `WorldFolderMigration` warning, adds 30 seconds to startup, and may prevent the world from loading. After editing the map externally, start an isolated copy with the pinned Paper version, let its migration finish, stop it cleanly, and copy these metadata files back before rebuilding the image.
