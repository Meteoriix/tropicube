...
The enabled `fallenkingdoms` template uses private ports 25660–25669 and injects `MAP_ID=cactus`. The 1v1 and 2v2 beta queues reuse that image on 25670–25679 and 25680–25689 without keeping warm instances. Its image copies the immutable world from `dockerfiles/worlds/fallenkingdoms/`; player data, locks, and old level backups stay outside Git and the image context. Add another enabled `locations.maps` definition and change `MAP_ID` to deploy another map without changing game code. The embedded profile reserves 2–4 GiB. The development Docker configuration caps FK instances at 1–2 GiB and starts the regular template on demand, allowing one regular or beta FK instance to run beside Lobby and SheepWars within the local 8 GiB budget.
