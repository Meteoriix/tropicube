...
The world template is stored in Paper 26.3's current storage format. The Overworld data directory must retain `minecraft/world_gen_settings.dat`, `paper/metadata.dat`, and `paper/level_overrides.dat`. Removing any of them makes every ephemeral copy look like a vanilla world import, triggers the `WorldFolderMigration` warning, adds 30 seconds to startup, and may prevent the world from loading. After editing the map externally, start an isolated copy with the pinned Paper version, let its migration finish, stop it cleanly, and copy these metadata files back before rebuilding the image.
...
`./deploy.ps1` and `./deploy.sh` are the normal development commands. After image verification, they update all four `latest` tags and run `docker compose up -d --no-build --force-recreate --wait --wait-timeout 180 velocity`. They do not call `/maintenance`, create a Restic backup, or wait for a drain. Velocity's default `shutdown.stop-dynamic-servers: true` therefore immediately stops dynamic instances and running games.

## Reliable image lots, backups and acceptance

For a production release, `--skip-restart` / `-SkipRestart` builds and verifies only UTC candidate tags without modifying live tags. Activate a verified lot with `python3 tools/ops/tropicube_ops.py activate YYYYMMDD-HHMMSS`. On a running network, activation drains maintenance for one minute, requires an off-host backup, stops Velocity and requires all dynamic backends to stop. The manifest retains the previous four Tropicube images plus MySQL, Redis, and Docker proxy image IDs. Static service, proxy, and lobby health must pass before maintenance is lifted. Use `python3 tools/ops/tropicube_ops.py rollback YYYYMMDD-HHMMSS` to restore those images; maintenance stays enabled until data and gameplay are validated. Incompatible schemas require a matching backup, not just older images.
...
`diagnose` emits filtered JSON and a nonzero exit code on service, backup, disk or capacity alerts. The minute timer reports to the local journal; no external notification destination is configured. Run `python3 tools/ops/smoke_test.py <lot>` to boot and stop an isolated network containing Lobby, SheepWars, and Fallen Kingdoms with owned dynamic resources. Before opening, use Spark and diagnostics at 10/25/50 real players for an hour, plus twenty game cycles: no OOM, no sustained post-cleanup growth, tick p95 below 50 ms. These hardware/client acceptance tests remain mandatory.
