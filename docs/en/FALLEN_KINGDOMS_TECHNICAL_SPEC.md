...
At sudden death, respawns stop and the border reaches exactly 50 × 50 blocks at 45:00. At the limit, the team or teams with the most survivors win; an equal maximum is a draw. An administrative abort records no competitive statistics.

The default configuration validates a 30-second countdown, a 10-second result display, the ordered 5/25/30/45-minute phase deadlines, positive heart health, at least one kit, valid material identifiers, compatible layouts, non-overlapping coherent regions, and positions inside the playable map. Custom matches may vary only explicitly allowed gameplay settings; technical protections, no friendly damage, container preservation, and non-blocking I/O cannot be overridden.
