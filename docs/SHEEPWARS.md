...
6. Un bloc de laine lumineux flotte entre les deux bases. Une flèche qui traverse son volume consomme la cible et accorde immédiatement son bonus à tous les survivants connectés de l'équipe du tireur.
...
Chaque carte peut définir autant de centres candidats que nécessaire sous `locations.<carte>.powerups.target1`, `target2`, etc. Au lancement de la manche, un seul centre tiré aléatoirement affiche un `BlockDisplay` de laine avec un contour blanc lumineux. La couleur annonce le bonus : vert clair pour le soin, violet pour les flèches empoisonnées et bleu clair pour la vitesse. Les six cartes déployées fournissent cinq candidats chacune.
...
Le scoreboard affiche sous le titre tropical `🐑 SHEEPWARS` des sections aérées par de courts séparateurs et une ligne vide après la carte dans chaque état, que le joueur ait choisi une équipe ou soit spectateur. Cette ligne possède une entrée unique, un composant d'espacement et un format numérique vide afin que le client conserve sa hauteur sans afficher son score technique. Pendant l'attente, il indique l'effectif actuel et maximal, le minimum requis, le résultat courant du vote de carte avec son nombre de voix, l'équipe, la classe et le kit du joueur. En partie, il présente le temps restant, la carte, les survivants par équipe, l'équipe ou le statut spectateur, la classe, le kit, les éliminations et les moutons lancés. Le kit se rafraîchit immédiatement après sa sélection et après un changement de langue. La tablist reprend les identités `🐑 SHEEPWARS` et `🌴 TROPICUBE` sur deux lignes, puis adapte son pied à l'attente, au lancement, au jeu ou à la fin de partie. Les textes proviennent de TropicubeCore et sont disponibles en français, anglais, espagnol et allemand.
